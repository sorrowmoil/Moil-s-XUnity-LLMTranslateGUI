...
void MainWindow::addToGlossaryHistory(const QString& path) {
    ...
    // Extract current history items / 提取当前历史项目
    QStringList items;
    for (int i = 0; i < glossaryCombo->count(); ++i) {
        items << glossaryCombo->itemText(i);
    }
    
    // Remove duplicates and add new path to top / 移除重复项并将新路径添加到顶部
    items.removeAll(path);
    items.insert(0, path);
    
    // Limit history size to 5 items / 限制历史记录大小为5项
    while (items.size() > 5) {
        items.removeLast();
    }
    
    // Update combo box / 更新组合框
    glossaryCombo->clear();
...
void MainWindow::onSelectGlossary() {
    // Open file dialog to select glossary file / 打开文件对话框选择术语表文件
...
void MainWindow::setupUi() {
    ...
    sysLabelLayout->addStretch(); // 顶起上面的元素
...
void MainWindow::loadConfigToUi() {
    ...
    // Populate UI elements with config values / 使用配置值填充UI元素
...
AppConfig MainWindow::getUiConfig() {
    ...
    // Save glossary history / 保存术语表历史记录
    QStringList history;
    for(int i = 0; i < glossaryCombo->count(); ++i) {
...
void MainWindow::toggleControls(bool running) {
    ...
    // Port is always locked while running / 运行时端口始终锁定
    portEdit->setEnabled(!running);
    
    // Core control buttons / 核心控制按钮
    stopBtn->setEnabled(running);
    hudBtn->setEnabled(running);

    if (running) {
        // === Hot Reload Mode === / === 热重载模式 ===
        // Allow changing parameters on the fly / 允许动态更改参数
        apiAddressCombo->setEnabled(true);
        apiKeyEdit->setEnabled(true);
        modelCombo->setEnabled(true);
        tempSpin->setEnabled(true);
        systemPromptEdit->setEnabled(true);
        chkGlossary->setEnabled(true);
        
        // Threads/Context: Can be hot-patched depending on implementation, 
        // usually safe to keep disabled or enabled depending on preference.
        // We will keep them enabled for flexibility in this perfect version.
        // 线程数/上下文：根据实现可以进行热修补，通常根据偏好保持禁用或启用都是安全的。
        // 在这个完美版本中，我们保持启用以获得灵活性。
        threadSpin->setEnabled(true); 
        contextSpin->setEnabled(true);
        
        // Glossary path changing might be risky during active translation ops, disable path selection
        // 在活跃的翻译操作期间更改术语表路径可能有风险，禁用路径选择
        glossaryCombo->setEnabled(false);
        btnSelectGlossary->setEnabled(false);

        // Turn Start button into Reload button (Blue) / 将启动按钮转换为重载按钮（蓝色）
        startBtn->setText(STR_RELOAD[m_currentLang]);
        startBtn->setObjectName("btnReload"); 
        startBtn->setEnabled(true); 
    } else {
        // === Stopped Mode === / === 停止模式 ===
        // Unlock everything / 解锁所有控件
        apiAddressCombo->setEnabled(true);
        apiKeyEdit->setEnabled(true);
        modelCombo->setEnabled(true);
        tempSpin->setEnabled(true);
        ...
        btnSelectGlossary->setEnabled(true);

        // Restore Start button (Green) / 恢复启动按钮（绿色）
        startBtn->setText(STR_START[m_currentLang]);
        startBtn->setObjectName("btnStart"); 
        startBtn->setEnabled(true);
    }

    if(btnOpenAuto) btnOpenAuto->setEnabled(true);
    
    // Force style refresh to apply new objectName color / 强制刷新样式以应用新的objectName颜色
...
void MainWindow::onStartClicked() {
    ...
    if (m_isServerRunning) {
        // === Hot Reload Logic === / === 热重载逻辑 ===
        AppConfig oldCfg = server->getConfig();
        
        // 1. Detect Model Switch / 1. 检测模型切换
        if (oldCfg.model_name != newCfg.model_name) {
            logArea->append(QString(LOG_MODEL_SWITCH[m_currentLang])
                            .arg(oldCfg.model_name)
                            .arg(newCfg.model_name));
        } else {
            logArea->append(QString(LOG_MODEL_SAME[m_currentLang]).arg(newCfg.model_name));
        }

        // 2. Apply Config / 2. 应用配置
        server->updateConfig(newCfg);
        logArea->append(LOG_RELOADED[m_currentLang]);
        
        // 3. Visual Feedback (Blink) / 3. 视觉反馈（闪烁）
        ...
        // 4. Auto-Test Connection (Crucial for verifying new keys/models) / 4. 自动测试连接（对于验证新密钥/模型至关重要）
        logArea->append(LOG_AUTO_TESTING[m_currentLang]);
        onTestConfig();
...
void MainWindow::onLoadConfig() {
    ...
    if (!fileName.isEmpty()) {
        ...
        // Apply configuration to UI / 将配置应用到UI
        apiAddressCombo->setCurrentText(cfg.api_address);
        apiKeyEdit->setText(cfg.api_key);
        modelCombo->setCurrentText(cfg.model_name);
        portEdit->setText(QString::number(cfg.port));
        ...
        // Update glossary history / 更新术语表历史记录
        glossaryCombo->clear();
        if (!cfg.glossary_history.isEmpty()) {
            glossaryCombo->addItems(cfg.glossary_history);
        }
